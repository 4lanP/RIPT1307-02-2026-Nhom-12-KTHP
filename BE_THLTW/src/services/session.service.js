...
async function scan(qr_code) {
  ...
  try {
    ...
    if (qrRes.rows.length === 0) throw { statusCode: 404, message: 'Mã QR không hợp lệ hoặc đã bị vô hiệu hóa' };
    ...
    if (table.status === 'OCCUPIED') {
      throw { statusCode: 409, message: 'Bàn đang có khách, vui lòng liên hệ nhân viên' };
    }
...
async function getSession(session_id) {
  ...
  if (rows.length === 0) throw { statusCode: 404, message: 'Session không tồn tại' };
  return rows[0];
}
...
async function getTableActiveSession(table_id) {
  ...
  if (sessionRes.rows.length === 0) {
    throw { statusCode: 404, message: 'Bàn không có session đang hoạt động' };
  }
...
async function checkoutCash(session_id, amount) {
  ...
  try {
    ...
    if (sessionRes.rows.length === 0) throw { statusCode: 404, message: 'Session không tồn tại' };
...
async function cancelOrderItem(order_item_id, cancel_reason) {
  ...
  try {
    ...
    if (updateRes.rows.length === 0) throw { statusCode: 404, message: 'Order item không tồn tại' };
